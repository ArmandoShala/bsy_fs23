...
// ---------- section  
    ...
    long page_size = getpagesize();    // sysconf(_SC_PAGESIZE);
    (void) printf("Page size: %ld\n", page_size);
...
// ---------- section 
    ...
    int num_pages = 4;
    long mem_size = num_pages * page_size;

    void *p_mem = malloc(mem_size);

    if (p_mem == NULL) {
        (void) printf("Failed to allocate memory\n");
        exit(EXIT_FAILURE);
    }

    (void) printf("Allocated %ld bytes of memory at address %p\n", mem_size, p_mem);
...
// ---------- section 
    ...
    void *p_buf = aligned_alloc(page_size, mem_size);
    ...
    (void) printf("mincore:\n");
    (void) printf("Before buffer access:\n");

    // call mincore and check each page in the block
    if (mincore(p_mem, mem_size, p_buf) == 0) {
        // print the status of each page
        for (int i = 0; i < num_pages; i++) {
            (void) printf("Page %d: %s\n", i, (p_buf[i] & 1) ? "in memory" : "not in memory");
        }
    } else {
        (void) printf("mincore failed");
    }

    // Print the contents of the allocated memory block
    (void) printf("Memory samples: ");
    for (size_t i = 0; i < 64; i++) { // first 64 as samples
        (void) printf("%c", *((char *)p_buf + i));
    }
    (void) printf("\n");

    // ToDo 5: write something into the reserved buffer
    // Access the allocated memory block using pointer arithmetic
    for (size_t i = 0; i < mem_size; i++) {
        *((char *)p_buf + i) = 'a' + (i % 26);
    }

    // Print the contents of the allocated memory block
    (void) printf( "After buffer access:\n" );
